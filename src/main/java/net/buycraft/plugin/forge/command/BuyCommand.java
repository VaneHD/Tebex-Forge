...
public class BuyCommand implements Command<CommandSource> {
    ...
    public int run(CommandContext<CommandSource> context) throws CommandSyntaxException {
        ...
        ForgeMessageUtil.sendMessage(context.getSource(), new StringTextComponent("                                            ").applyTextStyles(TextFormatting.STRIKETHROUGH));
        ForgeMessageUtil.sendMessage(context.getSource(), new StringTextComponent(ForgeMessageUtil.format(plugin,"To view the webstore, click this link: ")).applyTextStyle(style -> {
            style.setColor(TextFormatting.GREEN);
            style.setClickEvent(new ClickEvent(ClickEvent.Action.OPEN_URL, plugin.getServerInformation().getAccount().getDomain()));
        }));
        ForgeMessageUtil.sendMessage(context.getSource(), new StringTextComponent(plugin.getServerInformation().getAccount().getDomain()).applyTextStyle(style -> {
